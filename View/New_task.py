...
class New_task(tk.Toplevel): # 继承Toplevel类
    ...
    def createPage(self):
        ...
        self.email_option.current(1) # 设置下拉列表默认显示的值，0为 numberChosen['values'] 的下标值
        ...
        status_list = self.read_file.read_case_file().col_values(15)
        testcase_list = self.read_file.read_case_file().col_values(3)
        ...
        def select_case_list(event):
            ...
            for q in temp:  # 过滤有效的case_ID
                if status_list[q] == '有效':
                    case_list.append(testcase_list[q]+"***"+casename_list[q])

            self.caseID_list = case_list
        ...
        # 待执行用例ID列表
        ttk.Label(self.labelframe,text='待执行用例ID列表').grid(row=5, stick=W, column=6,pady=5,padx=0)
    ...
    def summit_data(self):
        ...
        self.new_task_list = [task_id,self.task_name.get(),self.Task_description.get('1.0',END),self.select_item_intername,
                              list(self.exe_caseID.get(0,END)),self.setup_SQL.get('1.0',END).replace("\n"," ").strip(),
                              self.teardown_SQL.get('1.0',END).replace("\n"," ").strip(),self.email.get(),'待执行']
        try:
            for i in range(0,len(self.new_task_list)):
                sheet_write.write(rows,i,str(self.new_task_list[i]))
            w_xls.save(self.read_file.save_task_file)
            showinfo(title="信息",message="任务新增成功！")
            self.destroy()
            ...
            showerror(title='错误', message="任务保存失败！%s"%e)
    ...
    def new_task_id(self):
        ...
        if case_list != []:
            ...
            task_id_ori[1] = "%04d" % int(task_id_ori[1])  # 前面补0，保留4位
